'use strict';
// ============================================================
// world.js — renderer, terrain, biomes, city, sky, underworld,
// sky islands, portals, garage, ground queries
// ============================================================
...
terrainGeo.rotateX(-Math.PI / 2);
{
  ...
  const sand = srgb(0xd9c58a), grass = srgb(0x55953f), rock = srgb(0x8a8578),
        ...
        dryGrass = srgb(0x9aa04f), deepGreen = srgb(0x2f6e33),
        mud = srgb(0x6b5a45), cliff = srgb(0x7a7268);
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    else if (h < 10.5) c.copy(rock).lerp(cliff, 0.2);
    else c.copy(snow);
    const t = tempAt(x, z), mo = moistAt(x, z);
    if (h > WATER_Y + 0.9) {
      if (t < 0.28) c.lerp(snow, (0.28 - t) / 0.28 * 0.9);
      else if (t > 0.68 && mo < 0.4) {
        c.lerp(sand, 0.55); c.lerp(redrock, Math.max(0, (t - 0.68)) * 1.4);
...
const puddles = [];
{
  const puddleGeo = new THREE.CircleGeometry(1.5, 12);
  puddleGeo.rotateX(-Math.PI / 2);
  const puddleMat = standard(0x3a6a8a, 0.05, 0.8, { transparent: true, opacity: 0 });
  for (let i = 0; i < 60; i++) {
    const x = (Math.random() - 0.5) * (CITY_EDGE * 2);
    const z = (Math.random() - 0.5) * (CITY_EDGE * 2);
    const p = new THREE.Mesh(puddleGeo, puddleMat.clone());
...
const facadeMats = [
  facadeMaterial('#8d99a6', '#31404f', 0.4),
  facadeMaterial('#6e7f8d', '#22303c', 0.35),
  facadeMaterial('#a89f91', '#3a3f46', 0.45),
  facadeMaterial('#4f5d6e', '#1d2733', 0.5),
  facadeMaterial('#9a8f83', '#333a41', 0.3),
  facadeMaterial('#7a8a96', '#2a3845', 0.38),
  facadeMaterial('#b0a898', '#404550', 0.42),
];
...
const snowConiferMat = lambert(0x5a8a5a);
...
// Grass tufts
const grassTufts = [];
const grassBladeGeo = new THREE.ConeGeometry(0.06, 0.4, 4);
...
function addTree(x, y, z, s, kind) {
  ...
  if (kind === 'cactus') {
    ...
    const arm2 = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.8, 6), cactusMat);
    arm2.position.set(-0.45, 2.0, 0); arm2.rotation.z = -0.4;
    g.add(body, arm, arm2);
  } else if (kind === 'palm') {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      f.position.set(0.5, 4.4, 0);
      f.rotation.y = (i / 7) * Math.PI * 2;
      f.rotation.x = 0.45;
      g.add(f);
    }
    const coconuts = new THREE.Mesh(new THREE.SphereGeometry(0.12, 6, 4), lambert(0x6b4a2b));
    coconuts.position.set(0.2, 4.1, 0.1);
    g.add(coconuts);
    ...
    const tr = new THREE.Mesh(trunkGeo, trunkMat);
    tr.position.y = 1.2; tr.castShadow = true;
    const lv = new THREE.Mesh(coneGeo, Math.random() < 0.3 ? snowConiferMat : coniferMat);
...
// Rocks scattered in countryside
{
  let placed = 0;
  while (placed < 80) {
...
// Grass tufts in moist areas
{
  const grassGeo = new THREE.ConeGeometry(0.06, 0.35, 4);
  for (let i = 0; i < 400; i++) {
    const x = (Math.random() - 0.5) * (MAP - 100);
    const z = (Math.random() - 0.5) * (MAP - 100);
    if (cityMask(x, z) > 0.02) continue;
...
const auroraMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `varying vec2 vUv; uniform float uTime; uniform float uIntensity;
    void main() {
      ...
      float alpha = band * uIntensity * 0.25;
      gl_FragColor = vec4(col, alpha);
    }`,
  transparent: true, side: THREE.DoubleSide, depthWrite: false, fog: false,
});
const auroraGeo = new THREE.PlaneGeometry(600, 80, 32, 8);
const aurora = new THREE.Mesh(auroraGeo, auroraMat);
aurora.position.set(0, 180, -300);
...
const islands = [];
{
  ...
  for (const d of defs) {
    ...
    for (let i = 0; i < Math.floor(d.r / 8); i++) {
      ...
      addTree(d.x + Math.cos(a) * rr, d.y + 0.9, d.z + Math.sin(a) * rr,
              0.7 + Math.random() * 0.6, 'broadleaf');
    }
  }
}
...
const lavaPools = [];
{
  ...
  for (const [lx, lz, lr] of spots) {
    ...
    p.rotation.x = -Math.PI / 2;
    p.position.set(lx, uwFloorHeight(lx, lz) + 0.15, lz);
    scene.add(p);
  ...
  for (let i = 0; i < 50; i++) {
    ...
    cr.position.set(x, onCeil ? -37 : uwFloorHeight(x, z) + 1.2, z);
...
function addPortal(x, y, z, dest, colorHex, label) {
  ...
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(4, 0.4, 10, 32),
...
function groundAt(x, z, y) {
  if (y < UW_LIMIT) {
    return Math.abs(x) < UW_HALF && Math.abs(z) < UW_HALF ? uwFloorHeight(x, z) : uwFloorHeight(x, z);
  }
...
// ---------- Environment reflections (PBR) ----------
{
  ...
  const envSky = new THREE.Mesh(new THREE.SphereGeometry(50, 16, 10), new THREE.ShaderMaterial({
    ...
    fragmentShader: `varying vec3 vPos; uniform vec3 uTop; uniform vec3 uBottom;
      void main() {
        vec3 d = normalize(vPos);
        gl_FragColor = vec4(mix(uBottom, uTop, pow(clamp(d.y, 0.0, 1.0), 0.55)), 1.0);
      }`,
    side: THREE.BackSide,
  }));
  envScene.add(envSky);
